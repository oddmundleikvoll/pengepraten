  ...
  return (
    <button
      ...
      className={`
        ...
        ${completed
          ? 'bg-amber-warm-soft border-amber-warm/40 scale-95 shadow-sm'
          : 'bg-paper-surface border-border hover:border-forest/40 hover:shadow-md hover:scale-102'
        }
        ${isCurrentWeek && !completed ? 'ring-2 ring-forest/40 ring-offset-1' : ''}
    ...
    >
      {/* Icon badge */}
      <div className="absolute -top-2 -right-2 text-forest">{icon}</div>

      {/* Week number */}
      <div className={`text-xs font-bold mb-1 ${completed ? 'text-amber-warm' : 'text-ink-subtle'}`}>
        Uke {week}
      </div>

      {/* Amount */}
      <div className={`text-base font-extrabold leading-tight ${completed ? 'text-amber-warm' : 'text-ink'}`}>
        {amount.toLocaleString('nb-NO')} kr
      </div>

      {/* Running total */}
      <div className={`text-xs mt-1 ${completed ? 'text-amber-warm' : 'text-ink-subtle'}`}>
        Total: {total.toLocaleString('nb-NO')} kr
      </div>

      {/* Checkmark overlay */}
      {completed && (
        <div className="absolute top-1 left-1 w-5 h-5 bg-forest rounded-full flex items-center justify-center shadow-sm animate-in zoom-in-75">
